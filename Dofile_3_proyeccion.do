...
gen p_matri_7_`b' = matri_exp_6_`a'*(1 - r_drop2_exp`a'  + t_tras_exp2`a')+matri_exp_7_`a'*t_desap_exp2`a'

forval x = 7/10 {
local y = `x' + 1
gen p_matri_`y'_`b' = matri_exp_`x'_`a'*(1 - r_drop2_exp`a' - t_desap_exp2`a'+ t_tras_exp2`a') + matri_exp_`y'_`a'*t_desap_exp2`a'
}
...
merge 1:1 codooii year using "3. Data\Datasets_intermedios\matri_proy_2020.dta"

global epm epm_ma2018 epm_exp2018 epm_ue2018 epm_csr2018 `varbid'
...
export excel CODOOII year `var4' metodo metodo_ue ue exp1 ma  mat_CSR `matbid' /// 
using "4. Codigos\Output\Proyeccion_porUGEL.xls", ///
	sheet("`var4'") sheetreplace firstrow(varlabels)

collapse (sum) ue `var4' exp1 ma mat_CSR `matbid' ///
	epm_ma2018  epm_exp2018 epm_ue2018 epm_csr2018 `varbid' , by(year)
